...
export function Hero({ hero }: { hero: HeroContent }) {
  ...
  return (
    <section className="mx-auto grid max-w-6xl gap-6 px-4 py-8 md:grid-cols-[1.2fr,0.8fr] md:gap-10 md:py-16 lg:py-24">
      <div>
        ...
        <div className="mt-6 flex flex-col gap-3 md:flex-row md:mt-8">
          <Link href="/#projects" className="rounded-xl bg-emerald-500 px-5 py-3 text-center font-semibold text-slate-950 hover:bg-emerald-600">View Projects</Link>
          <Link href="/services" className="rounded-xl border border-slate-300 px-5 py-3 text-center font-semibold dark:border-slate-700">My Services</Link>
        </div>
      </div>
      <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900 md:rounded-3xl md:p-6">
        {hero.image ? (
          <img src={hero.image} alt="Kgomotso Mamogale" className="w-full rounded-xl object-cover h-40 md:h-48" />
        ) : (
          <>
